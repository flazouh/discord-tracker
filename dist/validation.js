 ...
 */
export function validateStepNumber(stepNumber, totalSteps) {
    if (stepNumber <= 0) {
    ...
    if (stepNumber > totalSteps) {
        throw new TrackerError(`Step number (${stepNumber}) cannot be greater than total steps (${totalSteps})`, "STEP_NUMBER_EXCEEDS_TOTAL");
    }
}
/**
...
export function validateRepository(repository) {
    ...
    if (!repoPattern.test(repository)) {
        throw new TrackerError("Invalid repository format. Expected: owner/repo", "INVALID_REPOSITORY");
    }
}
/**
