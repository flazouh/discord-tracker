...
async function main() {
    ...
    if (!githubOutputPath) {
        ...
        try {
            ...
            fs.writeFileSync(path.join(process.cwd(), 'error.log'), `error=${errorMsg}\nsuccess=false`);
        }
        catch (e) {
    ...
    const [action, prNumber, prTitle, author, repository, branch, stepNumber, totalSteps, stepName, status, additionalInfo, errorMessage, botToken, channelId,] = args;
    let tracker;
    try {
    ...
    let result; // Placeholder for the result of tracker operations
    try {
        switch (action) {
            case 'init':
