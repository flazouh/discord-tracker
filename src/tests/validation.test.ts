...
describe("Validation Functions", () => {
	...
	describe("validateBotToken", () => {
		it("should pass for a valid bot token", () => {
			...
			expect(() =>
				validateBotToken("1234567890.abcdefghijklmnopqrstuvwxyz.abcdef"),
			).not.toThrow();
		});
		...
		it("should throw error for invalid bot token format", () => {
			expect(() => validateBotToken("invalid_token")).toThrow(
	...
	describe("validateChannelId", () => {
		...
		it("should throw error for a channel ID containing letters", () => {
			expect(() => validateChannelId("123abc456")).toThrow(
				"Invalid Discord channel ID format",
			);
		});

		it("should throw error for invalid scientific notation", () => {
			expect(() => validateChannelId("1.2E+invalid")).toThrow(
				"Invalid Discord channel ID format",
			);
		});
	});
	...
	describe("validateStepNumber", () => {
		...
		it("should throw error for zero step number", () => {
			expect(() => validateStepNumber(0, 5)).toThrow(
				TrackerError.invalidStepNumber(0),
			);
		});

		it("should throw error for negative step number", () => {
			expect(() => validateStepNumber(-1, 5)).toThrow(
				TrackerError.invalidStepNumber(-1),
			);
		});
