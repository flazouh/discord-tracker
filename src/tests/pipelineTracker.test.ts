...
describe('PipelineTracker Integration Tests', () => {
  ...
  it('should successfully create PipelineTracker instance with valid credentials', () => {
    expect(() => new PipelineTracker(VALID_BOT_TOKEN, VALID_CHANNEL_ID)).not.toThrow();
  });

  it('should throw error when bot token is empty', () => {
    expect(() => new PipelineTracker(INVALID_BOT_TOKEN, VALID_CHANNEL_ID)).toThrow(
      TrackerError.missingRequiredInput('discord_bot_token')
    );
  });

  it('should throw error when channel ID is invalid', () => {
    expect(() => new PipelineTracker(VALID_BOT_TOKEN, INVALID_CHANNEL_ID)).not.toThrow();
  });
});
